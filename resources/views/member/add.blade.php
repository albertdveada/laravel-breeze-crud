<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-7">
      <div class="mt-5 text-white">
        <h2 class="font-semibold text-xl mt-1">Add Members</h2>
      </div>
      <div class="mt-5" x-data="{ imageUrl: '/storage/profil_image.png' }">
        <form action="{{ route('member.adding') }}" method="POST" class="flex flex-col md:flex-row gap-6" enctype="multipart/form-data">
          @csrf
          ...
          <div class="md:w-1/2">
            ...
            <div class="mt-4">
              ...
              <input id="name" name="name" type="text" class="block w-full mt-1 px-4 py-2 bg-gray-800 text-gray-100 border border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-500 transition-all" :value="old('name')" required autocomplete="name" />
              <x-input-error :messages="$errors->get('name')" class="mt-2 text-red-400" />
            </div>

            <div class="mt-4">
              <x-input-label for="phone" :value="__('Phone')" class="text-gray-300" />
              <input id="phone" name="phone" type="tel" class="block w-full mt-1 px-4 py-2 bg-gray-800 text-gray-100 border border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-500 transition-all" :value="old('phone')" required autocomplete="tel" minlength="9" />
